...
Docker is the recommended way to run the backend in an isolated, reproducible environment.
...
```bash
cd backend
docker build -t seas-backend .
```

The first build downloads Python dependencies and compiles the `phe` C extension (GMP-backed). This takes 2–4 minutes. Subsequent builds are cached and much faster.

### 2. Create a data volume

The backend uses SQLite. A named volume keeps the database intact across container restarts:

```bash
docker volume create seas-data
```
...
docker run -d \
  ...
  -p 8000:8000 \
  -v seas-data:/data \
  -e DATABASE_URL="sqlite+aiosqlite:////data/seas.db" \
  seas-backend
```
...
| `--name seas-backend` | Name the container for easy reference |
| `-p 8000:8000` | Expose the API on localhost:8000 |
| `-v seas-data:/data` | Persist the SQLite database in a named volume |
| `-e DATABASE_URL=...` | Point the app at the volume-backed database path |
...
Run the seeder inside the running container:

```bash
docker exec seas-backend python seed.py
```
...
```bash
# View live logs
docker logs -f seas-backend

# Open a shell inside the container
docker exec -it seas-backend bash

# Stop the container
docker stop seas-backend

# Start it again (database is preserved in the volume)
docker start seas-backend

# Remove the container (volume and image are kept)
docker rm seas-backend

# Remove everything including the database volume
docker rm seas-backend
docker volume rm seas-data
```
